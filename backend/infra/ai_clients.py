...
class AIClientConfig:
    ...
    ocr_base_url: Optional[str] = "http://192.168.118.80:18083"
    ocr_model: str = "paddle"
    ...
    def from_env(cls, base_dir: Path | None = None):
        ...
        return cls(
            ...
            ocr_base_url=_get_config_value("OCR_BASE_URL", dotenv_values, "http://192.168.118.80:18083").rstrip("/"),
            ocr_model=_get_config_value("OCR_MODEL", dotenv_values, "paddle"),
...
class OpenAICompatibleClient(BaseAIClient):
    ...
    def generate_text(self, prompt: str, system_prompt: str | None = None, model: str | None = None) -> Dict[str, Any]:
        def execute() -> Dict[str, Any]:
            ...
            candidates = [model] if model else self.get_text_model_candidates()
            errors: list[str] = []
            for model_name in candidates:
                try:
                    ...
                    try:
                        from backend.infra.metrics import record_llm_call
                        record_llm_call(duration, success=True)
                    except Exception:
                        pass
                    return {
                        "mode": "live_text",
                    ...
                    try:
                        from backend.infra.metrics import record_llm_call
                        record_llm_call(duration, success=False)
                    except Exception:
                        pass
    ...
    def _extract_ocr_text_via_remote(self, image_path: Path) -> str:
        def execute() -> str:
            ...
            endpoint = f"{self.config.ocr_base_url.rstrip('/')}/parse"
            mime_type = "image/jpeg"
            ...
            try:
                with image_path.open("rb") as f:
                    response = requests.post(
                        ...
                        data={
                            "ocr_engine": self.config.ocr_model or "paddle",
                            "output_format": "html",
                            "async_mode": "false",
                        },
